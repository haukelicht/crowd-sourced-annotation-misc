---
title: Analyze BBA models fitted to simulated anti-elite codings datasets varying n
  and n[i]
author: "Hauke Licht"
date: "4/16/2019"
...
elite_plot_caption <- bquote("Anti-elitism classification simulated with"~
        pi==.(round(sim_fitted$elite$sim_params$pi, 3))~","~
        theta[j0]~"~"~f[Beta](.(round(sim_fitted$elite$sim_params$alpha0, 3)),.(round(sim_fitted$elite$sim_params$beta0, 3)))~
        "and"~
        theta[j1]~"~"~f[Beta](.(round(sim_fitted$elite$sim_params$alpha1, 3)),.(round(sim_fitted$elite$sim_params$beta1, 3)))
    )
...
The true prevalence $\pi$ has been simulated at `r sim_fitted$elite$sim_params$pi`.
...
ggplot(data.frame(x = seq(0,1,.1)), aes(x)) + 
  ...
  scale_colour_discrete(
    labels = c(
      bquote(theta[.*0]%~%f[Beta](.(round(sim_fitted$elite$sim_params$alpha0, 3)), .(round(sim_fitted$elite$sim_params$beta0, 3))))
      , bquote(theta[.*1]%~%f[Beta](.(round(sim_fitted$elite$sim_params$alpha1, 3)), .(round(sim_fitted$elite$sim_params$beta1, 3))))
    )
  )
```
...
tibble(
  ...
  ggplot(aes(x = theta0, y = theta1)) +
    ...
    geom_point() +
    scale_x_continuous(limits = range(sim_fitted$elite$sim_params$theta0) + c(-.05, 0)) + 
    scale_y_continuous(limits = range(sim_fitted$elite$sim_params$theta1) + c(-.05, .05)) +
    labs(
...
In total, I have randomly sampled 500 items from a Bernoulli distribtion with the simulated $\pi$ value of `r sim_fitted$elite$sim_params$pi`.
...
elite_sim_codings <- map_df(sim_fitted$elite$fitted, function(g) {
  map_df(g, function(l) {
    l$data %>% 
...
p_elite_deviance_chains + labs(
    ...
    , subtitle = "Results obtained by fitting BBA model to simulated anti-elitism codings"
    , color = "Chain"
    , x = "Iterations"
    , y = NULL
    , caption = elite_plot_caption
  )
```
...
for(g in seq_along(sim_fitted$elite$fitted)) {
  for (n in 1:8) {
    
    nm <- sprintf("p_elite_shrinkage_n[i]=%s_n=%s", n+2, seq(200, 500, 50)[g])
    
    png(file.path(file_path, "plots", paste0(nm, ".png")))
    coda::gelman.plot(sim_fitted$elite$fitted[[g]][[n]]$fit[,"deviance"], main = gsub("_", " ", nm))
    dev.off()
  } 
}
```
...
for(g in seq_along(sim_fitted$elite$fitted)) {
  for (n in 1:8) {
    ...
    nm <- sprintf("p_elite_autocorr_n[i]=%s_n=%s", n+2, seq(200, 500, 50)[g])
    
    png(file.path(file_path, "plots", paste0(nm, ".png")))
    coda::autocorr.plot(sim_fitted$elite$fitted[[g]][[n]]$fit[,"deviance"], ask = FALSE, main = gsub("_", " ", nm), lag.max = 50)
    dev.off()
  } 
}
```
...
```{r plot pi chains}
elite_pis <- map_df(sim_fitted$elite$fitted, function(g) {
  map_df(g, function(l) {
...
```{r plot pi posterior}
p_elite_pi_post <- ggplot(elite_pis, aes(x = est)) +
  geom_density(color = NA, fill = "grey", alpha = .75) +
  geom_vline(aes(xintercept = sim_fitted$elite$sim_params$pi), color = "red") + 
  facet_grid(
    rows = vars(sample_size)
    , cols = vars(n_i)
    , switch = "y"
...
```{r get c posteriors}
elite_cs <- map_df(sim_fitted$elite$fitted, function(g) {
  map_df(g, function(l) {
...
The above plot says little about the significance of the reductions in mean PCUs, however.
...
So, for instance, for $n = 200$, if we aggregate four instead of three judgments per items, this leads to a decrease in PCU in only about `r (1-elite_pcu_deltas_sum[1,4])*100`% of cases.
We are `r (1-elite_pcu_deltas_sum[4,4])*100`% (`r (1-elite_pcu_deltas_sum[5,4])*100`%) certain, however, that collecting seven (eight) judgments instead of three judgmentes per item results in an average decrease in item-levelU, which meets the 5% confidence level criterion. 
...
(Note the scaling of the x-axis, 90% of values are in the [.7,1] range.)
...
There exist no substantial accuracy differentials across values of $n$, however. (Only 90%-CIs get tighter as $n$ is increased, which is due to higher precision resulting from computing accuracy from a increasingly larger samples.)
...
- Finally, the *F1-score* that combines recall and precision into one metric[^f1] increases significantly as $n_i$ is increased. This is because in the denominator low Precision depresses the F1-score.
...
p_elite_residuals_by_class + 
    labs(
      ...
      , y = NULL
      , fill = "True class:"
      , caption = "Grey vertical line cutting the x-axis at .5 indicates classification threshold: items with absoult residual error > .5 are misclassified."  #elite_plot_caption
    )
```
...
Indeed, as true items make up only between `r range(elite_sim_codings$prop_true)*100`% of all items in the respective subsets, the amount of misclassification resulting from poor true-positive detection rates is actually not too grave as the following plot shows more accurately.
...
p_elite_residuals + 
  labs(
      ...
      , y = NULL
      # , caption = elite_plot_caption
      , caption = "Grey vertical line cutting the x-axis at -.5 and .5, respectively, indicate classification thresholds: items with residual error < -.5 and > .5 are misclassified."  #elite_plot_caption
    )
```
...
p_elite_voted_quality +
  labs(
    ...
    , subtitle = "Results obtained by inducing majority winner (w/ random tie-breaking) in simulated anti-elitism codings"
    , y = NULL
    , x = expression(n[i])
    , caption = elite_plot_caption
  )
...
A stiking result of ther classification performances of majorty voting is that we get no false-positives.
Hence, both the Precision and TNR for all models are perfect (false-positives factor in the denominator of both metrics).
And what is more, because of the strong class imbalance, the accuracies are also very close to perfect.
Indeed the F1-score is a much more informative metric in presence of such intense class imbalance.
...
These differences are illustrated for F1-scores in the following plot.
...
p_elite_f1_compared + 
  labs(
    title = "Mean differences and 90%-CIs between F1-scores induced by majority voting and model-based classification"
...
This shows that while majority voting outperforms model-based classifications in terms of F1-scores for all values of $n$ (though not significantly for all values), we see that for higher $n_i$ the model tends to perform (significantly) better.
Specifically, for $n_i > 6$ (differences for $n_i = 6$ may be induced by random tiebreaking in majority voting), we are 95% certain for all values of $n$ that the model-based classification yield higher F1-scores than does majority voting.
Note that the choice of comparing the F1-scores of model and majority voting induced comparisons was theoretically informed.
In presence of such strong class imbalance, the accuracy is not a good performance criterion.
By taking both precision and recall into account, we can achieve high F1-scores only if our aggregation method performs also reasonable well in correctly classifying true-positive items---a job the BBA model is doing better than the majority vote for $n_i > 6$ in the simulated data.
